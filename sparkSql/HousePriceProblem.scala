...
object HousePriceProblem {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val session = SparkSession.builder().appName("HousePriceSol").master("local[1]").getOrCreate()

    val realEstate = session.read.option("header", "true").option("inferSchema", value = true).csv("in/RealEstate.csv")
